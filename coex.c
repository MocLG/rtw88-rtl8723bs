...
void rtw_coex_8723bs_scan_workaround(struct rtw_dev *rtwdev)
{
	...
	/* Start with staging's non-connected scan/auth workaround: PS-TDMA
	 * type 8 with TDMA off, PTA antenna path, and coex table type 2.
	 */
...
void rtw_coex_8723bs_pre_auth_h2c(struct rtw_dev *rtwdev)
{
	...
	/* Vendor rtl8723bs v5.2.17 sends the BT_MP 0x67 queries while still
	 * scanning, then only replays BT_INFO + PS-TDMA type 8 immediately
	 * before start_clnt_join()/auth. Keep the auth window free of extra
	 * 0x67 query/response traffic so the firmware's C2H path is not busy
	 * when the first directed management TX goes out.
	 */
	rtw_fw_query_bt_info(rtwdev);
	rtw_fw_coex_tdma_type(rtwdev, 0x08, 0x00, 0x00, 0x00, 0x00);
	rtw_fw_coex_tdma_type(rtwdev, 0x08, 0x00, 0x00, 0x00, 0x00);
 ...
 * TDMA changes; those are covered by scan_workaround / force_assoc_pta_ant
...
static void rtw_coex_action_wl_only(struct rtw_dev *rtwdev)
{
	 ...
	 * routed BB_SEL_BTG away from the RF path used by scan, so
	 * post-assoc connection-quality TX/RX never reached the AP on-air.
	 * Keep this on the same antenna routing as scan and the connect-window
	 * override.
	 */
	rtw_coex_set_ant_path(rtwdev, false, COEX_SET_ANT_2G);
	rtw_coex_set_rf_para(rtwdev, chip->wl_rf_para_rx[0]);
