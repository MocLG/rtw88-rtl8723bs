			       ...
			       enum rtw_tx_queue_type queue)
{
	...
	if (trace_mgmt) {
		if (rtwdev->chip->id == RTW_CHIP_TYPE_8723B &&
		    rtw_hci_type(rtwdev) == RTW_HCI_TYPE_SDIO) {
			rtw_info(rtwdev,
				 "MGMT_TX_DEBUG: write_result stype=%s fc=0x%04x queue=%u txaddr=0x%08x skb_len=%u txsize=%zu ret=%d align=%lu txfree=%d/%d/%d/%d oqt=%d HISR=0x%08x TXDMA_STATUS=0x%08x TXPAUSE=0x%02x SDIO_TX_CTRL=0x%08x MACID_DROP=0x%08x MACID_SLEEP=0x%08x TXPKT_EMPTY=0x%04x RQPN=0x%08x RQPN_NPQ=0x%02x PQ_MAP=0x%04x QUEUE_CTRL=0x%02x FWTQ=0x%08x CR=0x%08x MSR=0x%02x BCN=0x%02x RCR=0x%08x RRSR=0x%08x PAD1=0x%08x BB_SEL=0x%08x SYS=0x%02x RF_CTRL=0x%02x\n",
				 rtw_sdio_mgmt_stype_name(tx_data->frame_control),
