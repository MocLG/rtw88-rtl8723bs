					...
					struct rtw_tx_desc *tx_desc)
{
	...
	rtw_info(rtwdev,
		 "MGMT_TX_DEBUG: prepared stype=%s fc=0x%04x queue=%u qsel=%u mac_id=%u len=%u skb_len=%u offset=%u pkt_offset=%u rate=%u rate_id=%u bw=%u seq=%u sn=%u report=%d use_rate=%d dis_fb=%d dis_qseq=%d en_hwseq=%d retry_en=%d retry_lmt=%u rts=%d sec=%u ch=%u hal_bw=%u pwr_idx=%d desc=%08x/%08x/%08x/%08x/%08x/%08x/%08x/%08x/%08x/%08x\n",
