 ...
 */
static void rtw_hmac_sha256_vector(u8 *key, size_t key_len, size_t num_elem,
		       ...
		       u8 *data, size_t data_len, u8 *buf, size_t buf_len)
{
	...
	while (pos < buf_len) {
		...
		if (plen >= SHA256_MAC_LEN) {
			rtw_hmac_sha256_vector(key, key_len, 4, addr, len,
					   &buf[pos]);
			pos += SHA256_MAC_LEN;
		} else {
			rtw_hmac_sha256_vector(key, key_len, 4, addr, len, hash);
