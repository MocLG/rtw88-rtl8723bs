...
s32 FillH2CCmd8723B(struct adapter *padapter, u8 ElementID, u32 CmdLen, u8 *pCmdBuffer)
{
	...
	printk(KERN_INFO "staging_h2c: %s id=0x%02x len=%u payload=",
	       __func__, ElementID, CmdLen);
	if (pCmdBuffer && CmdLen > 0) {
		dbg_len = CmdLen > sizeof(dbg_payload) ? sizeof(dbg_payload) : CmdLen;
		memcpy(dbg_payload, pCmdBuffer, dbg_len);
		for (i = 0; i < dbg_len; i++)
			printk(KERN_CONT "%02x ", dbg_payload[i]);
	}
	printk(KERN_CONT "\n");
